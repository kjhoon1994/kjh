...
<script type="text/javascript">
	$(function(){
		$("#delete").click(function(){
			...
			var f =document.teamlistForm
			f.action="<%=cp%>/team/delete.do";
			f.submit();
			}
		});
		...
		$("#update").click(function(){
			...
			if(confirm("팀을 변경하시겠습니가?")){
			var d =document.teamlistForm
			d.action="<%=cp%>/team/update.do";
			d.submit();
			}
		});
	...
	<div class="row">
		...
		<div class="col-lg-6">
			<form name="teamlistForm" method="post">
						...
						<table
							...
							<c:forEach var="dto" items="${list}">
								<tr>
									...
									<td>${dto.rownum}</td>
									<td><a
										href="<%=cp%>/team/teamarticle.do?teamId=${dto.teamId}&leagueId=${dto.leagueId}">${dto.teamName}</a></td>
									<td>${dto.coach}</td>
				...
				<button type="button"
					onclick="javascript:location.href='<%=cp%>/team/created.do?leagueId=${leagueId}'">팀
					추가</button>
				<button type="button" id="delete">팀 삭제</button>
				<button type="button" id="update">팀 정보변경</button>
